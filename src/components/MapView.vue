<template>
  <!-- eslint-disable vue/no-deprecated-slot-scope-attribute, vue/no-deprecated-v-bind-sync -->
  <div class="layout-container">
      ...
      <el-table :data="pointData" height="400">
        ...
        <el-table-column label="经度" width="100">
          <template #default="scope">
            <template v-if="typeof scope.row.longitude === 'number'">
              {{ scope.row.longitude.toFixed(6) }}
            </template>
            <template v-else-if="typeof scope.row.lng === 'number'">
              {{ scope.row.lng.toFixed(6) }}
            </template>
            <template v-else-if="typeof scope.row.longitude === 'string'">
              {{ scope.row.longitude }}
            </template>
            <template v-else-if="typeof scope.row.lng === 'string'">
              {{ scope.row.lng }}
            </template>
        ...
        <el-table-column label="纬度" width="100">
          <template #default="scope">
            <template v-if="typeof scope.row.latitude === 'number'">
              {{ scope.row.latitude.toFixed(6) }}
            </template>
            <template v-else-if="typeof scope.row.lat === 'number'">
              {{ scope.row.lat.toFixed(6) }}
            </template>
            <template v-else-if="typeof scope.row.latitude === 'string'">
              {{ scope.row.latitude }}
            </template>
            <template v-else-if="typeof scope.row.lat === 'string'">
              {{ scope.row.lat }}
            </template>
            <template v-else>--</template>
          </template>
    ...
    <!-- 区域分析结果对话框 -->
    <el-dialog
      v-model="showDrawAnalysisDialog"
      title="区域分析结果"
      width="80%"
      destroy-on-close
      top="5vh"
      class="draw-analysis-dialog"
      :append-to-body="true">
      <div class="analysis-summary">
        <div class="shape-info">
          <template v-if="currentDrawnShape && currentDrawnShape.type === 'circle'">
            <h4>圆形区域</h4>
            <p>半径: {{ (drawAnalysisRadius/1000).toFixed(2) }} 公里</p>
            <p>面积: {{ (drawAnalysisArea/1000000).toFixed(2) }} 平方公里</p>
          </template>
          <template v-else-if="currentDrawnShape && currentDrawnShape.type === 'polygon'">
            <h4>多边形区域</h4>
            <p>面积: {{ (drawAnalysisArea/1000000).toFixed(2) }} 平方公里</p>
          </template>
        </div>
      ...
      <el-tabs type="border-card">
        ...
        <el-tab-pane label="充电站">
          <div v-if="drawAnalysisResults.charging.length === 0" class="empty-data">
            <el-empty description="区域内未找到充电站"></el-empty>
          </div>
          <div v-else>
            <h4>找到 {{ drawAnalysisResults.charging.length }} 个充电站</h4>
            <el-table 
              :data="drawAnalysisResults.charging" 
              stripe 
              border 
              size="small" 
              height="250" 
              ...
              <el-table-column label="操作" width="80" fixed="right">
                <template #default="scope">
                  <el-button size="small" type="primary" @click="locateAnalysisPoint(scope.row)">定位</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
        ...
        <el-tab-pane label="停车场">
          <div v-if="drawAnalysisResults.parking.length === 0" class="empty-data">
            <el-empty description="区域内未找到停车场"></el-empty>
          </div>
          <div v-else>
            <h4>找到 {{ drawAnalysisResults.parking.length }} 个停车场</h4>
            <el-table 
              :data="drawAnalysisResults.parking" 
              stripe 
              border 
              size="small" 
              height="250" 
        ...
        <!-- 推荐选址标签页 -->
        <el-tab-pane label="推荐选址">
          <div v-if="drawAnalysisResults.recommended.length === 0" class="empty-data">
            <el-empty description="区域内未找到推荐选址点"></el-empty>
          </div>
          <div v-else>
            <h4>找到 {{ drawAnalysisResults.recommended.length }} 个推荐选址</h4>
            <el-table 
              :data="drawAnalysisResults.recommended" 
              stripe 
              border 
              size="small" 
              height="250" 
              style="width: 100%"
              :resize-observer="null">
              <el-table-column prop="name" label="位置" min-width="120"></el-table-column>
        ...
        <!-- 人口分布标签页 -->
        <el-tab-pane label="人口分布">
          <div v-if="drawAnalysisResults.population.length === 0" class="empty-data">
            <el-empty description="区域内未找到人口分布点"></el-empty>
          </div>
          <div v-else>
            <h4>找到 {{ drawAnalysisResults.population.length }} 个人口分布点</h4>
            <el-table 
              :data="drawAnalysisResults.population" 
              stripe 
              border 
              size="small" 
              height="250" 
              style="width: 100%"
              :resize-observer="null">
              <el-table-column prop="name" label="区域" min-width="120"></el-table-column>
              <el-table-column prop="longitude" label="经度" width="100"></el-table-column>
              <el-table-column prop="latitude" label="纬度" width="100"></el-table-column>
              <el-table-column prop="population" label="人口数" width="80"></el-table-column>
              <el-table-column label="操作" width="80" fixed="right">
                <template #default="scope">
                  <el-button size="small" type="primary" @click="locateAnalysisPoint(scope.row)">定位</el-button>
                </template>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="showDrawAnalysisDialog = false">关闭</el-button>
          <el-button type="primary" @click="searchDrawnArea; searchAreaFromTianditu()">重新搜索</el-button>
          <el-button type="danger" @click="clearDrawAnalysis">清除分析</el-button>
          <el-button type="warning" @click="deleteCurrentShape">删除绘制形状</el-button>
        </div>
      </template>
    </el-dialog>
...
export default {
  ...
  mounted() {
    ...
    this.initMap().then(() => {
      ...
      // 添加全局错误处理，捕获 ResizeObserver 循环错误
      ...
      // 优化性能，减少ResizeObserver错误
      if (this.map) {
        // 延迟地图刷新，避免频繁触发resize事件
  ...
  methods: {
    ...
    activateTool(tool) {
      ...
      if (this.activeTool) {
        ...
        ElMessage.info('已停用工具')
      }

      // 如果点击的是当前已激活的工具，则仅停用不再激活新工具
      if (tool === this.lastActiveTool) {
      ...
      if (tool === 'point') {
        ...
        ElMessage.info('请在地图上点击添加新的点位')
      } else if (tool === 'polygon') {
        this.initializePolygonDrawing();
        ElMessage.info('请在地图上点击绘制多边形，双击结束绘制')
      } else if (tool === 'circle') {
        this.initializeCircleDrawing();
        ElMessage.info('请在地图上点击选择圆心，然后拖动确定半径')
      }
    },
    showHeatmap() {
    ...
    openServiceRangeDialog() {
      ...
      if (this.analysisMode === 'recommended') {
        ...
        // 推荐选址模式下，确保充电站数据已加载，因为需要这些数据进行分析
    ...
    startPointSelection() {
      ...
      // 添加ESC键取消选点的事件监听
      const cancelSelectionHandler = (e) => {
        if (e.key === 'Escape' && this.isSelectingPoint) {
          // 取消选点模式
    ...
    handleMapClickAddPoint(e) {
      if (!this.activeTool || this.activeTool !== 'point') {
        return
      }
      
      const latlng = e.latlng
      
      // 如果是从导入菜单选择"从地图添加"，则使用已选择的数据类型
      if (this.importDataType && (this.importDataType === 'charging' || this.importDataType === 'parking')) {
        // 直接使用已选的数据类型
        this.addPointWithNamePrompt(latlng, this.importDataType)
        // 使用完后重置
    ...
    confirmImport() {
      if (this.importMethod === 'map') {
        ...
        this.$confirm('请选择要添加的数据类型', '选择数据类型', {
          ...
          this.lastActiveTool = 'point'
          // 更改鼠标样式
          document.getElementById('map').style.cursor = 'crosshair'
          // 显示提示
          ElMessage.info('请在地图上点击添加充电站点位')
        }).catch(action => {
          if (action === 'cancel') {
            // 用户选择了停车场
    ...
    initDrawTools() {
      ...
      this.map.on(L.Draw.Event.CREATED, (event) => {
        ...
        // 保存当前绘制的形状信息
        this.currentDrawnShape = {
          type: event.layerType,
          layer: layer
        };
        
        // 计算区域面积
        this.calculateDrawnArea();
        
        // 搜索区域内的数据
        this.searchDrawnArea();
        
        // 为绘制的图形添加点击事件，显示删除确认框
        layer.on('click', (e) => {
          // 阻止事件冒泡，防止触发地图点击事件
          L.DomEvent.stopPropagation(e);
          
          // 显示删除确认对话框
          ElMessageBox.confirm('是否要删除当前绘制的形状?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            ...
            ElMessage.info('已保留绘制形状');
          });
        });
        
        // 从天地图API搜索数据并显示对话框
        this.searchAreaFromTianditu();
      });
      
      // 注册绘图取消事件
      this.map.on('draw:cancel', () => {
        this.drawing = false;
        document.getElementById('map').style.cursor = '';
    ...
    // 计算绘制区域面积
    calculateDrawnArea() {
      if (!this.currentDrawnShape) return;
      
      if (this.currentDrawnShape.type === 'circle') {
        const circle = this.currentDrawnShape.layer;
        const radius = circle.getRadius();
        this.drawAnalysisRadius = radius;
        this.drawAnalysisArea = Math.PI * radius * radius;
      } else if (this.currentDrawnShape.type === 'polygon') {
        const polygon = this.currentDrawnShape.layer;
        ...
        if (latLngs && latLngs.length > 2) {
          ...
          // 转换为实际地理面积的近似值
          const scale = this.getMapScale(this.map);
          this.drawAnalysisArea = area * scale * scale;
        }
      }
    },
    ...
    getMapScale(map) {
      ...
      const targetPoint = L.point(centerPoint.x + 100, centerPoint.y);
      const targetLatLng = map.containerPointToLatLng(targetPoint);
      return this.calculateDistance(center.lat, center.lng, targetLatLng.lat, targetLatLng.lng) / 100;
    },
    ...
    searchDrawnArea() {
      ...
      // 检查人口分布数据
      if (this.populationPoints && this.populationPoints.length > 0) {
        // 根据形状类型检查点是否在区域内
        if (this.currentDrawnShape.type === 'circle') {
          const circle = this.currentDrawnShape.layer;
          ...
          this.drawAnalysisResults.population = this.populationPoints.filter(point => {
      ...
      // 注意：这里不再设置showDrawAnalysisDialog为true
      // 这个操作由searchAreaFromTianditu完成
    },
    
    // 从天地图API搜索区域内的数据
    async searchAreaFromTianditu() {
      if (!this.currentDrawnShape) return;
      ...
      try {
        ...
        if (this.currentDrawnShape.type === 'circle') {
          const circle = this.currentDrawnShape.layer;
          const center = circle.getLatLng();
          const radius = circle.getRadius() / 1000; // 转换为公里
          
          bounds = `${center.lng - radius/100},${center.lat - radius/100},${center.lng + radius/100},${center.lat + radius/100}`;
        } else if (this.currentDrawnShape.type === 'polygon') {
          const polygon = this.currentDrawnShape.layer;
          const latLngs = polygon.getLatLngs()[0];
        ...
        // 搜索充电站
        const chargingResponse = await axios.get('http://api.tianditu.gov.cn/v2/search', {
          params: {
            type: 'query',
            tk: this.tiandituKey,
            postStr: JSON.stringify({
        ...
        // 搜索停车场
        const parkingResponse = await axios.get('http://api.tianditu.gov.cn/v2/search', {
          params: {
            type: 'query',
            tk: this.tiandituKey,
            postStr: JSON.stringify({
        ...
        // 处理充电站搜索结果
        if (chargingResponse.data && chargingResponse.data.pois) {
          const stations = chargingResponse.data.pois.map(poi => {
            const station = {
              id: poi.hotPointID || poi.uid,
              name: poi.name,
              address: poi.address || '暂无地址',
          ...
          // 过滤掉已有的结果
          const existingIds = new Set(this.drawAnalysisResults.charging.map(s => s.id));
          const newStations = stations.filter(s => !existingIds.has(s.id));
          
          // 添加到结果中
          this.drawAnalysisResults.charging = [...this.drawAnalysisResults.charging, ...newStations];
        }
        
        // 处理停车场搜索结果
        if (parkingResponse.data && parkingResponse.data.pois) {
          const parkings = parkingResponse.data.pois.map(poi => {
            const parking = {
              id: poi.hotPointID || poi.uid,
              name: poi.name,
              address: poi.address || '暂无地址',
          ...
          // 过滤掉已有的结果
          const existingIds = new Set(this.drawAnalysisResults.parking.map(p => p.id));
          const newParkings = parkings.filter(p => !existingIds.has(p.id));
          
          // 添加到结果中
          this.drawAnalysisResults.parking = [...this.drawAnalysisResults.parking, ...newParkings];
        }
      } catch (error) {
        console.error('从天地图API搜索区域数据失败:', error);
      } finally {
        // 使用nextTick确保DOM更新后再显示对话框
        this.$nextTick(() => {
          // 显示分析结果对话框
          this.showDrawAnalysisDialog = true;
        });
      }
    },
    ...
    locateAnalysisPoint(point) {
      ...
      // 创建临时标记
      if (this.tempLocationMarker) {
        this.tempLocationMarker.setLatLng([lat, lng]);
      } else {
        // 根据点类型选择图标
        let iconUrl;
        if (point.type === '充电站') {
        ...
        this.tempLocationMarker = L.marker([lat, lng], {
          icon: icon,
          zIndexOffset: 1000
        }).addTo(this.map);
      }
      ...
      const popupContent = `
        <div class="poi-popup">
          <h3>${point.name || '未命名点位'}</h3>
          <p><strong>类型：</strong>${point.type || '未知'}</p>
          <p><strong>经度：</strong>${typeof lng === 'number' ? lng.toFixed(6) : lng}</p>
          <p><strong>纬度：</strong>${typeof lat === 'number' ? lat.toFixed(6) : lat}</p>
          ${point.address ? `<p><strong>地址：</strong>${point.address}</p>` : ''}
        </div>
      `;
      
      // 绑定弹出窗口并打开
      this.tempLocationMarker.bindPopup(popupContent).openPopup();
      
      // 显示提示
      ElMessage.success(`已定位到: ${point.name || '未命名点位'}`);
    },
    
    // 清除绘制分析结果
    clearDrawAnalysis() {
      // 先关闭对话框
      this.showDrawAnalysisDialog = false;
      ...
      setTimeout(() => {
        ...
        this.drawAnalysisRadius = 0;
        this.drawAnalysisArea = 0;
        
        ElMessage.success('已清除绘制分析结果');
      }, 100);
    },
    deleteCurrentShape() {
      // 删除当前绘制的形状
      if (this.currentDrawnShape) {
        this.drawnItems.removeLayer(this.currentDrawnShape.layer);
        this.currentDrawnShape = null;
        ElMessage.success('已删除当前绘制的形状');
      }
    },
    initializePolygonDrawing() {
      ...
      // 开始多边形绘制
      if (this.drawControl.polygon) {
        this.drawing = true;
        this.drawControl.polygon.enable();
      } else {
        ElMessage.error('多边形绘制工具初始化失败');
        this.activeTool = null;
        this.lastActiveTool = null;
      }
    },
    ...
    initializeCircleDrawing() {
      ...
      // 开始圆形绘制
      if (this.drawControl.circle) {
        this.drawing = true;
        this.drawControl.circle.enable();
      } else {
        ElMessage.error('圆形绘制工具初始化失败');
        this.activeTool = null;
        this.lastActiveTool = null;
      }
    }
...
/* 绘制分析对话框样式 */
.draw-analysis-dialog .analysis-summary {
  margin-bottom: 20px;
  padding: 15px;
  background-color: #f5f7fa;
  border-radius: 4px;
...
.draw-analysis-dialog .shape-info {
  display: flex;
  flex-direction: column;
}

.draw-analysis-dialog .shape-info h4 {
  margin-top: 0;
  margin-bottom: 10px;
  color: #303133;
}

.draw-analysis-dialog .shape-info p {
  margin: 5px 0;
  color: #606266;
}

.draw-analysis-dialog .empty-data {
  padding: 40px 0;
}

.draw-analysis-dialog .recommend-score {
  font-weight: bold;
  color: #409EFF;
}

.draw-analysis-dialog .el-tabs__content {
  padding: 15px;
}

.draw-analysis-dialog h4 {
  margin-top: 0;
  margin-bottom: 15px;
  color: #303133;
}
